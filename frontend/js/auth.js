...
// Global flash helper: type = 'success' | 'error' | 'warning' | 'info'
function showFlash(message, type = "success", duration = 4000) {
  ...
  flash.className = "mango-flash";
  flash.style.cssText = `position:fixed; top:20px; left:50%; transform:translateX(-50%); background:${c.bg}; color:${c.text}; padding:12px 18px; border-radius:8px; z-index:9999; box-shadow:0 6px 18px rgba(0,0,0,0.12); font-weight:700; max-width:90%; text-align:center;`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (function setupBackendBanner() {
    ...
    if (!document.getElementById(bannerId)) {
      ...
      banner.style.cssText =
        "display:none; position:fixed; top:10px; left:50%; transform:translateX(-50%); background:#ffcc00; color:#000; padding:10px 16px; border-radius:6px; z-index:9999; box-shadow:0 2px 6px rgba(0,0,0,0.15); font-weight:600;";
      ...
      document
        .getElementById("backend-retry-btn")
        .addEventListener("click", async () => {
          document.getElementById("backend-status-msg").textContent =
            "Checking...";
          try {
            const ok = await (window.ensureApiReady
              ? window.ensureApiReady()
              : null);
            if (ok) {
              document.getElementById("backend-status-msg").textContent =
                "Backend available";
  ...
  const handleRegister = async (event, role) => {
    ...
    if (!isEmailVerified) {
      showFieldError(
        "email",
        "✓ Please verify your email with OTP before submitting",
      );
      hasError = true;
    }
    ...
    if (role === "BROKER") {
      ...
      if (!data.platform_fee_paid) {
        ...
        showFieldError(
          "platform_fee_paid",
          "✓ Please check this box to agree and continue",
        );
        btn.disabled = false;
        btn.innerText = originalText;
        return;
      ...
      if (
        !tradeLicenseInput ||
        !tradeLicenseInput.files ||
        tradeLicenseInput.files.length === 0
      ) {
        console.log("Trade license file not selected"); // Debug
        showFieldError(
          "tradeLicenseFile",
          "✓ Please upload your trade license document",
        );
        btn.disabled = false;
        btn.innerText = originalText;
        return;
    ...
    if (role === "BROKER") {
      ...
      for (const [key, value] of Object.entries(data)) {
        if (key !== "trade_license_file") {
          // Skip the file field as we'll handle it separately
      ...
      try {
        ...
        if (!base) {
          result = {
            success: false,
            message:
              "Cannot reach backend. Make sure the backend server is running.",
          };
        } else {
    ...
    if (result.user_id) {
      ...
      // Success - store minimal session info if provided
      try {
        if (result.token) {
        ...
        if (result.role_specific_id)
          localStorage.setItem("role_id", result.role_specific_id);
      ...
      // If the registrant is a BROKER, show a confirmation flash and redirect to login
      const isBroker =
        role === "BROKER" ||
        (result.role && result.role.toUpperCase() === "BROKER");
      if (isBroker) {
        showFlash(
          "Registration successful — we will verify your license soon. Please log in after 24 hours.",
          "success",
        );
        setTimeout(() => {
        ...
        const isFarmer =
          role === "FARMER" ||
          (result.role && result.role.toUpperCase() === "FARMER");
        if (isFarmer) {
          showFlash(
      ...
      // Parse error and show on correct field (use more specific phrase matching)
      ...
      } else if (lowerMsg.includes("account holder")) {
        // Banking-related informational message
        showFlash(
          "Note: Banking details were accepted. " + errorMsg,
          "warning",
        );
      } else if (lowerMsg.includes("bank account")) {
        showFlash(
          "Note: Banking details were accepted. " + errorMsg,
          "warning",
        );

        // IFSC-related errors: do not mark the IFSC field invalid — accept any IFSC/bank details
      } else if (
        lowerMsg.includes("location") ||
        lowerMsg.includes("market area") ||
        lowerMsg.includes("city")
      ) {
        showFieldError("market_area", "Location is required");
        showFieldError("city", "Location is required");
      } else {
        // Generic error - show as flash
        showFlash("Error: " + errorMsg, "error");
      }
...
async function verifyOTP() {
  ...
  if (otp.length !== 6) {
    showFlash("Enter 6-digit OTP", "error");
    return;
  }
...
function closeForgotPasswordModal() {
  ...
  document
    .querySelectorAll("#forgotPasswordModal .field-error")
...
function setupForgotPasswordHandlers() {
  ...
  // Cancel buttons
  if (cancelBtn1)
    cancelBtn1.addEventListener("click", closeForgotPasswordModal);
  if (cancelBtn2) {
  ...
  if (cancelBtn3)
    cancelBtn3.addEventListener("click", closeForgotPasswordModal);
