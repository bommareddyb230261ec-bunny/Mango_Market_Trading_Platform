...
const TransactionManager = {
        ...
        const wAsTx = data.weighments.map((w) => {
          const totalCost =
            typeof w.total_cost === "number"
              ? w.total_cost
              : (w.actual_weight_tons || 0) *
                1000 *
                (w.final_price_per_kg || 0);
          const commission =
            typeof w.commission === "number" ? w.commission : 0;
          const netPayable =
            typeof w.net_payable === "number"
              ? w.net_payable
              : Math.max(totalCost - commission, 0);
          return {
            id: `w-${w.id}`,
  ...
  renderTransactions() {
    ...
    if (this.filteredTransactions.length === 0) {
      grid.innerHTML =
        '<div class="loading-card"><p>No transactions found</p></div>';
      return;
    }

    grid.innerHTML = this.filteredTransactions
      .map((tx) => {
        const idAttr = String(tx.id).replace(/'/g, "\\'");
        const weightText =
          typeof tx.actual_weight === "number"
            ? `${tx.actual_weight.toFixed(2)} tons`
            : "N/A";
        const netPay =
          typeof tx.net_payable === "number"
            ? `₹${tx.net_payable.toFixed(2)}`
            : "₹0.00";
  ...
  showDetails(transactionId) {
    const tx = this.allTransactions.find(
    ...
    const weight =
      typeof tx.actual_weight === "number"
        ...
        : "N/A";
    const price =
      typeof tx.market_price_at_sale === "number"
        ? "₹" + tx.market_price_at_sale.toFixed(2)
        : "₹0.00";
    const totalAmount =
      typeof tx.total_cost === "number"
        ? tx.total_cost.toFixed(2)
        : (
            (tx.actual_weight || 0) *
            (tx.market_price_at_sale || 0) *
            1000
          ).toFixed(2);

    document.getElementById("detail-farmer-name").textContent =
      tx.farmer_name || `Farmer #${tx.farmer_id}`;
    ...
    document.getElementById("detail-commission").textContent =
      typeof tx.commission === "number"
        ...
        : "₹0.00";
    document.getElementById("detail-upi-id").textContent =
      tx.upi_transaction_id || "-";
    document.getElementById("detail-payment-proof").innerHTML =
      tx.payment_proof_url
        ? `<a href="${encodeURI(tx.payment_proof_url)}" target="_blank" rel="noopener">View Proof</a>`
        : "-";
    document.getElementById("detail-net-payable").textContent =
      typeof tx.net_payable === "number"
        ? "₹" + tx.net_payable.toFixed(2)
        : "₹0.00";
  ...
  getTransactionStatus(tx) {
    ...
    if (
      tx._is_weighment ||
      (typeof tx.actual_weight === "number" && tx.actual_weight > 0)
    ) {
      return "PENDING";
    }
    // Default to PENDING
  ...
  searchTransactions() {
    const searchTerm = document
      ...
      .value.toLowerCase();
    const transactionStatus = document.getElementById(
      "transaction-status-filter",
    ).value;

    this.filteredTransactions = this.allTransactions.filter((t) => {
      const matchesSearch = (t.farmer_name || `Farmer #${t.farmer_id}`)
        .toLowerCase()
        .includes(searchTerm);

      const txStatus = this.getTransactionStatus(t);
      const matchesStatus =
        !transactionStatus || txStatus === transactionStatus.toUpperCase();
    ...
    try {
      ...
      if (data.success && data.farmer) {
        return {
          ...
          account_holder:
            data.farmer.account_holder_name || data.farmer.name || "-",
