...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (verified !== "true") {
    console.log(
      "[Host Dashboard] Access denied - redirecting to host_access.html",
    );
...
async function approveBroker(brokerId, brokerName) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE}/api/host/brokers/${brokerId}/approve`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function rejectBroker(brokerId, brokerName) {
  const reason = prompt(
    `Enter reason for declining ${brokerName}:`,
    "License document is invalid or incomplete",
  );
  ...
  try {
    console.log(
      "[Host Dashboard] Rejecting broker:",
      brokerId,
      "Reason:",
      reason,
    );
    ...
    const response = await fetch(
      `${API_BASE}/api/host/brokers/${brokerId}/reject`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
function openStatisticsPage() {
  console.log("[Host Dashboard] Opening analytics dashboard...");
  window.location.href = "view_statics.html";
}
