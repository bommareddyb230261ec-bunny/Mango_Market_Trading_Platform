...
const BrokerDashboard = {
  ...
  async loadDashboard() {
    const response = await fetch(
      BROKER_CONFIG.API_BASE + BROKER_CONFIG.ENDPOINTS.DASHBOARD,
      {
        method: "GET",
    ...
    if (!data.success)
      throw new Error(data.message || data.error || "Unable to load dashboard");

    this.currentBroker = data.broker || {};
    this.sellRequests = this.sortRequests(
      Array.isArray(data.sell_requests) ? data.sell_requests : [],
    );
    this.transactions = Array.isArray(data.transactions)
      ? data.transactions
      : [];
    this.weighments = Array.isArray(data.weighments) ? data.weighments : [];
    this.marketPrices = Array.isArray(data.market_prices)
      ? data.market_prices
      : [];
  ...
  renderBrokerInfo() {
    ...
    const location =
      [place.market_area, place.district, place.state]
        .filter(Boolean)
    ...
    this.setText(
      "commission-display-text",
      `${this.formatMoney(commission)} per ₹100`,
    );

    const commissionInput = document.getElementById("market-commission");
    if (commissionInput)
      commissionInput.value = commission ? commission.toFixed(2) : "";
  },

  renderStats() {
    const pendingRequests = this.sellRequests.filter(
  ...
  renderPrices() {
    ...
    if (this.marketPrices.length === 0) {
      container.innerHTML =
        '<div class="empty-card">No prices set yet. Add your first variety above.</div>';
      return;
    }

    container.innerHTML = this.marketPrices
      .map((price) => {
  ...
  renderVarietyDropdown() {
    ...
    if (
      current &&
      Array.from(dropdown.options).some((option) => option.value === current)
    ) {
  ...
  renderRequests() {
    ...
    const pending = this.sellRequests.filter(
      (req) => String(req.status || "").toUpperCase() === "PENDING",
    );
    if (pending.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="empty-table">No pending sell requests.</td></tr>';
      return;
    }
  ...
  renderTransactions() {
    ...
    if (rows.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="9" class="empty-table">No transactions yet.</td></tr>';
      return;
    }
  ...
  async updateCommission(event) {
    ...
    const data = await this.postJson(
      BROKER_CONFIG.ENDPOINTS.UPDATE_COMMISSION,
  ...
  async updatePrice(event) {
    ...
    const existing = this.marketPrices.find(
      (item) => item.mango_variety === variety,
    );
    const payload = {
    ...
    const data = await this.postJson(
      BROKER_CONFIG.ENDPOINTS.UPDATE_PRICES,
      payload,
    );
  ...
  async addFruit(event) {
    ...
    const priceTon = Number(
      document.getElementById("fruit-initial-price")?.value,
    );
    const quantity = Number(
      document.getElementById("fruit-quantity")?.value || 100,
    );
    ...
    if (
      this.marketPrices.some(
        (item) =>
          String(item.mango_variety || "").toLowerCase() === name.toLowerCase(),
      )
    ) {
      this.notify(
        "This variety already exists. Update the existing price instead.",
        "error",
      );
      return;
    }
    ...
    const payload = {
      ...
      price_per_kg: priceTon / 1000,
      available_quantity:
        Number.isNaN(quantity) || quantity <= 0 ? 100 : quantity,
    };
  ...
  openAcceptModal(requestId) {
    const request = this.sellRequests.find(
    ...
    const marketPrice = this.marketPrices.find(
      (item) => item.mango_variety === request.variety,
    );
    ...
    this.setText(
      "acceptFarmerName",
      request.farmer_name || `Farmer #${request.farmer_id}`,
    );
    this.setText("acceptVariety", request.variety || "-");
    this.setText(
      "acceptQuantity",
      request.quantity_tons != null
        ? `${Number(request.quantity_tons).toFixed(2)} tons`
        : "-",
    );
    ...
    const defaultPrice =
      ...
      marketPrice?.price_per_kg ??
      "";
    if (priceInput)
      priceInput.value =
        defaultPrice !== "" ? Number(defaultPrice).toFixed(2) : "";
    if (dateInput)
      dateInput.value =
    ...
    if (help)
      help.textContent = marketPrice
        ? "Default uses current market price."
        : "No market price found. Enter agreed price manually.";
  ...
  async confirmAccept(event) {
    ...
    const agreedPrice = Number(
      document.getElementById("acceptAgreedPrice")?.value,
    );
    ...
    const response = await fetch(
      BROKER_CONFIG.API_BASE.replace(/\/$/, "") +
        `/sell-request/${requestId}/accept`,
      {
        method: "PUT",
        headers: this.headers(),
        credentials: "include",
  ...
  openRejectModal(requestId) {
    ...
    this.setText(
      "rejectFarmerInfo",
      request
        ? `Reject request from ${request.farmer_name || "Farmer"} for ${request.variety || "this variety"}.`
  ...
  async confirmReject(event) {
    ...
    const endpoint = BROKER_CONFIG.ENDPOINTS.UPDATE_REQUEST_STATUS.replace(
      "<id>",
      requestId,
    );
  ...
  openDeleteModal(priceId, variety) {
    ...
    this.setText(
      "deleteMessage",
      `Delete "${variety || "this variety"}" from your market?`,
    );
  ...
  bindEvents() {
    ...
    document
      ...
      ?.addEventListener("submit", (event) =>
        this.updateCommission(event).catch((error) =>
    ...
    document
      ...
      ?.addEventListener("submit", (event) =>
        this.updatePrice(event).catch((error) =>
          this.notify(error.message, "error"),
        ),
      );
    document
      .getElementById("add-fruit-form")
      ?.addEventListener("submit", (event) =>
        this.addFruit(event).catch((error) =>
          this.notify(error.message, "error"),
        ),
      );
    document
      ...
      ?.addEventListener("submit", (event) =>
        this.confirmAccept(event).catch((error) =>
          this.notify(error.message, "error"),
        ),
      );
    document
      ...
      ?.addEventListener("submit", (event) =>
        this.confirmReject(event).catch((error) =>
          this.notify(error.message, "error"),
        ),
      );
    document
      ...
      ?.addEventListener("click", () =>
        this.confirmDelete().catch((error) =>
          this.notify(error.message, "error"),
        ),
      );
    ...
    document.addEventListener("click", (event) => {
      ...
      if (deleteBtn)
        this.openDeleteModal(
          deleteBtn.dataset.deletePrice,
          deleteBtn.dataset.variety,
        );
  ...
  switchSection(section) {
    ...
    document
      .getElementById(`${this.activeSection}View`)
    ...
    document
      ...
      .forEach((item) =>
        item.classList.toggle(
          "active",
          item.dataset.section === this.activeSection,
        ),
      );
    const titles = {
      overview: "Dashboard",
  ...
  sortRequests(requests) {
    return [...requests].sort((a, b) => {
      const bTime = new Date(
        b.created_at || b.date || b.preferred_date || 0,
      ).getTime();
      const aTime = new Date(
        a.created_at || a.date || a.preferred_date || 0,
      ).getTime();
      return (
