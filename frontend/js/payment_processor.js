...
const PaymentProcessor = {
  ...
  async loadTransactionFromUrl() {
    try {
      ...
      const transactionId =
        urlParams.get("transactionId") || urlParams.get("transactionid");
  ...
  buildUPIUrl(options = {}) {
    ...
    const { upi_id, farmer_name, amount, order_id, transaction_id } =
      paymentData;

    // Validate UPI ID exists
    if (!upi_id || upi_id === "-" || upi_id === "") {
      this.showError(
        "Farmer UPI ID not available. Cannot initiate UPI payment.",
      );
      return null;
    }
    ...
    console.log(
      "   am (Amount):",
      includeAmount ? formattedAmount : "(manual)",
    );
  ...
  async openUPIApp(upiUrl) {
    ...
    const upiWindow = window.open(
      upiUrl,
      "_blank",
      "location=yes,height=570,width=520,scrollbars=yes,status=yes,toolbar=no,menubar=no",
    );
  ...
  isMobileDevice() {
    const isMobile =
      /Android|iPhone|iPad|iPod|webOS|BlackBerry|Opera Mini|IEMobile|MobileSafari/i.test(
        navigator.userAgent,
      );
    console.log(
      "📱 Mobile detection:",
  ...
  showPaymentConfirmationDialog() {
    ...
    dialog.style.cssText =
      "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 10000; display: flex; align-items: center; justify-content: center;";
  ...
  async confirmPaymentComplete() {
    ...
    const proofFile =
      proofInput && proofInput.files && proofInput.files[0]
        ? proofInput.files[0]
        : null;
    ...
    if (!proofFile) {
      this.showError(
        "Please upload the payment proof screenshot or PDF before submitting",
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        this.showSuccess(
          "🎉 Payment reference submitted successfully! Awaiting host verification.",
        );
        setTimeout(() => {
          window.location.href = "transactions.html";
        }, 2000);
  ...
  async displayPaymentDetails(data) {
    try {
      ...
      document.getElementById("payment-farmer-name").textContent =
        data.farmer_name || `Farmer #${data.farmer_id}`;
      document.getElementById("payment-farmer-phone").textContent =
        data.phone || "-";
      ...
      document.getElementById("payment-net-payable").textContent =
        "₹" + totalAmount.toFixed(2);

      {
        const grossAmount = Number(
          data.gross_amount ?? data.total_amount ?? data.amount ?? 0,
        );
        const commission = Number(data.commission ?? 0);
        const netPayable = Number(
          data.net_payable ??
            data.amount ??
            Math.max(grossAmount - commission, 0),
        );
        document.getElementById("payment-total-amount").textContent =
          "\u20b9" + grossAmount.toFixed(2);
        document.getElementById("payment-commission").textContent =
      ...
      document.getElementById("payment-upi-id").textContent =
        data.upi_id || "-";

      // Bank details - show as N/A since we're using UPI
      document.getElementById("payment-account-holder").textContent =
        data.farmer_name || "-";
