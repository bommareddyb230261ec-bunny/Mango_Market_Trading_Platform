...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const sellForm =
    ...
    document.getElementById("sellRequestForm");
  ...
  function requestStatusCount(requests, status) {
    return requests.filter(
      (request) => String(request.status || "").toUpperCase() === status,
    ).length;
  }
  ...
  async function loadLocations() {
    try {
      console.log(
        "Loading locations from:",
        `${API_BASE_URL}/farmer/locations`,
      );
      ...
      if (
        ...
        const uniqueDistricts = [
          ...new Set(data.locations.map((l) => l.district)),
        ];
        ...
        console.log(
          "Successfully loaded " + uniqueDistricts.length + " districts",
        );
      } else {
  ...
  async function fetchMarkets(district, sort) {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/farmer/markets?district=${encodeURIComponent(district)}&sort=${sort}`,
        {
          method: "GET",
  ...
  function renderMarkets(markets) {
    ...
    markets.forEach((m) => {
      ...
      const varietyList =
        m.varieties && m.varieties.length > 0 ? m.varieties : m.prices || [];
      ...
      const varietiesHtml =
        varietyList.length > 0
          ? varietyList
              .map((v) => {
                ...
                const priceVal =
                  v.price != null
                    ? Number(v.price)
                    : v.price_per_kg != null
                      ? Number(v.price_per_kg)
                      : null;
                const priceText =
                  priceVal != null ? `₹${priceVal.toFixed(2)}/kg` : "";
                return `
              ...
              .join("")
          : `<div style="color:#999;">Price TBA</div>`;
      ...
      if (btn) {
        btn.addEventListener("click", (ev) => {
          ...
          try {
            ...
            openSellModal(
              m.broker_id || m.brokerId || m.id,
              m.market_name || m.marketName || m.market_name,
            );
          } catch (err) {
            console.error(
              "openSellModal failed:",
              err,
              "brokerId:",
              m.broker_id,
              "name:",
  ...
  function renderAggregateVarieties(markets) {
    ...
    if (varieties.length === 0) {
      container.innerHTML =
        '<p style="color:#666">No varieties available in this district yet.</p>';
      return;
    }

    const chips = varieties
      .map((v) => {
  ...
  async function submitSellRequest(e) {
    ...
    const brokerField = document.getElementById("sellMarketId");
    const varietyField = document.getElementById("mangoVariety");
    const quantityField = document.getElementById("quantity");
    const dateField = document.getElementById("sellDate");
    ...
    if (
      ...
      if (errorMessage)
        errorMessage.textContent =
          "Select a variety, enter a quantity greater than zero, and choose a delivery date.";
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Sell request returned a non-JSON response:",
          responseText,
          parseError,
        );
      }
      ...
      if (res.ok) {
        try {
          localStorage.setItem(
            "mango_market_sell_requests_changed",
            String(Date.now()),
          );
        } catch (storageError) {
        ...
        if (successMessage) {
          successMessage.textContent =
            data.message || "Sell request submitted successfully";
        }
        if (successModal) {
  ...
  async function loadFarmerRequests() {
    const tbody =
      document.getElementById("myRequestsTableBody") ||
    ...
    try {
      ...
      if (
        ...
        if (tbody)
          showNoDataMessage(
            tbody,
            tbody.id === "myRequestsTableBody" ? 6 : 8,
            "No requests found.",
          );
        return;
      }
  ...
  async function loadAcceptedRequests() {
    ...
    try {
      ...
      if (tbody)
        showNoDataMessage(tbody, 6, "Unable to load accepted requests.");
    }
  }
  ...
  async function loadWeighments() {
    ...
    try {
      ...
      if (
        !data.success ||
        !Array.isArray(data.weighments) ||
        data.weighments.length === 0
      ) {
        showNoDataMessage(tbody, 11, "No weighment records available.");
        return;
      }

      tbody.innerHTML = data.weighments
        .map(
          (w) => `
                <tr>
  ...
  async function loadPayments() {
    ...
    try {
      ...
      const pendingAmount = pendingPayments.reduce((sum, payment) => {
        const amount = Number(
          payment.total_amount_debited || payment.total_amount || 0,
        );
      ...
      setDashboardText(
        "paymentStatus",
        pendingPayments.length > 0 ? `${pendingPayments.length} Open` : "Clear",
      );
  ...
  function renderStatusBadge(status, type = "request") {
    ...
    if (["ACCEPTED", "PAID", "COMPLETED"].includes(normalized)) {
      ...
      ["AWAITING_VERIFICATION", "INITIATED", "PENDING", "PROCESSING"].includes(
        normalized,
      )
    ) {
  ...
  window.showProofModal = function (proofUrl) {
    ...
    text.textContent = proofUrl
      ? "Proof is available below. If the preview cannot be loaded, open it in a new tab."
      : "No payment proof is attached to this transaction.";
    link.href = proofUrl || "#";
  ...
  async function sendBankOTP() {
    if (!bankEmail) {
      alert(
        "No email associated with your account. Please set an email in your profile and verify it.",
      );
      return;
    }
    const sendBtn = document.getElementById("sendBankOtpBtn");
  ...
  async function verifyBankOTP() {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/auth/verify-otp`, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ email: bankEmail, otp }),
      });
      const data = await res.json();
      if (data.success) {
        bankVerified = true;
        ...
        try {
          const bankRes = await fetch(`${API_BASE_URL}/farmer/bank`, {
            method: "GET",
            credentials: "include",
          });
          if (bankRes.ok) {
            ...
            if (bankData && bankData.success && bankData.bank) {
              if (bankData.verified) {
                if (bankData.bank.account_number)
                  document.getElementById("accountNum").value =
                    bankData.bank.account_number;
                if (bankData.bank.ifsc)
                  document.getElementById("ifscCode").value =
                    bankData.bank.ifsc;
                if (bankData.bank.upi)
                  document.getElementById("upiId").value = bankData.bank.upi;
              }
            }
          }
        } catch (err) {
          ...
          if (bankRes.ok) {
            ...
            if (bankData && bankData.success && bankData.bank) {
              if (bankData.verified) {
                ...
                if (bankData.bank.account_masked)
                  document.getElementById("accountNum").placeholder =
                    bankData.bank.account_masked;
                if (bankData.bank.ifsc_masked)
                  document.getElementById("ifscCode").placeholder =
                    bankData.bank.ifsc_masked;
                if (bankData.bank.upi_masked)
                  document.getElementById("upiId").placeholder =
                    bankData.bank.upi_masked;
              }
            }
          }
        } catch (err) {
            ...
            if (bankData.verified) {
              ...
              if (bankData.bank.account_number)
                document.getElementById("accountNum").value =
                  bankData.bank.account_number;
              if (bankData.bank.ifsc)
                document.getElementById("ifscCode").value = bankData.bank.ifsc;
              if (bankData.bank.upi)
                document.getElementById("upiId").value = bankData.bank.upi;

              bankVerified = true;
              document.getElementById("bankOtpSuccessMsg").style.display =
                "block";
              document.getElementById("bankVerifyWarning").style.display =
                "none";
              setBankControlsDisabled(false);
              document.getElementById("sendBankOtpBtn").style.display = "none";
            } else {
