...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function getHeaders() {
    return window.APIClient && typeof APIClient.getHeaders === "function"
  ...
  function setReadOnly(readonly) {
    editableFields.forEach((id) => {
      ...
      el.disabled = false;
      if (!readonly) el.classList.add("editing");
  ...
  async function loadProfile() {
    try {
      ...
      if (!res) {
        ...
        throw new Error(
          "Failed to fetch (" + hint + "). Is the backend running?",
        );
      }
